...
## ⚠️ LM Studio 업데이트 주의사항 (0.4.2 → 0.4.3+ 이슈)

> **최종 확인**: 2026-02-23 | **안정 버전**: 0.4.2

### 문제 현상

0.4.3 버전으로 업데이트 시 다음 문제 발생:
1. **앱 목록에 두 개 표시**: LM Studio 0.4.2와 0.4.3이 동시에 나타남
2. **충돌 발생**: 두 버전이 서로 충돌하여 정상 작동 안 함
3. **설정 꼬임**: 모델 경로, 서버 설정 등이 불안정

### 원인

- LM Studio는 AppImage 형식이지만 별도 설치 경로와 설정을 사용
- 새 버전이 기존 버전을 완전히 대체하지 않고 병렬 설치됨
- 두 버전이 같은 설정 파일(~/.lmstudio)을 공유하면서 충돌

### 현재 안정 구성 (server_rack)

| 항목 | 값 |
|------|-----|
| 버전 | **0.4.2** |
| 실행 파일 | `/home/yooha/Applications/LMStudio.AppImage` |
| 심볼릭 링크 | `/usr/local/bin/lmstudio` |
| 설정 폴더 | `~/.lmstudio` |
| 모델 경로 | `/mnt/nvme_4raid/model/lmstudio/models` (심볼릭: `~/.lmstudio/models`) |

### 업데이트 전 체크리스트

```bash
# 1. 현재 버전 확인
ls -la ~/Applications/LMStudio*.AppImage

# 2. 실행 중인 프로세스 확인
pgrep -af LMStudio

# 3. 설정 백업
cp -r ~/.lmstudio ~/.lmstudio.backup.$(date +%Y%m%d)
```
...
```bash
# 1. LM Studio 완전 종료
pkill -f LMStudio.AppImage
pkill -f lm-studio

# 2. 기존 AppImage 백업 (삭제 아님!)
mv ~/Applications/LMStudio.AppImage ~/Applications/LMStudio.0.4.2.AppImage.bak

# 3. 새 버전 다운로드 (웹에서 .AppImage 파일)
# → ~/Applications/ 에 저장
...
# 5. 기존 이름으로 심볼릭 링크 (중요!)
ln -sf ~/Applications/LMStudio-0.4.3-x86_64.AppImage ~/Applications/LMStudio.AppImage

# 6. 테스트 실행
~/Applications/LMStudio.AppImage --no-sandbox

# 7. 문제 발생 시 롤백
rm ~/Applications/LMStudio.AppImage
mv ~/Applications/LMStudio.0.4.2.AppImage.bak ~/Applications/LMStudio.AppImage
```

### 두 개 앱이 나타났을 때 해결법

```bash
# 1. 모든 LM Studio 프로세스 종료
pkill -f LMStudio
pkill -f lm-studio

# 2. 앱 목록 캐시 갱신
rm -rf ~/.local/share/applications/lmstudio*.desktop
update-desktop-database ~/.local/share/applications/

# 3. 시스템 앱 목록에서도 제거 (있다면)
...
- **안정성**: 현재 시스템에서 모든 기능 정상 작동
- **호환성**: Tool Calling, Speculative Decoding, Vision 모두 지원
- **위험 회피**: 0.4.3 업데이트 시 충돌 문제 재발 가능
